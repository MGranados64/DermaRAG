...
def ejecutar_pipeline_gradcam(modelo, ruta_img, temp_dir):
    ...
    plt.subplot(1, 3, 3)
    plt.bar(CLASES_NOMBRES, probs, color=['green', 'red', 'orange']) 
    ...
    if uploaded_file and modelo_cnn:
        ...
        if not nombre or localizacion is None:
            ...
            with st.status("🔄 Ejecutando Sistema Multiagente...", expanded=True) as status:
                ...
                task_med = (
                    f"DATOS DEL PACIENTE: {nombre}, {edad_val} años, {sexo if sexo else 'No esp.'}. ID: {id_paciente}.\n"
...
# ==============================================================================
                ...
                task_atencion_primaria = Task(
                    ...
                    - **Análisis de Atención (Grad-CAM):** [Interpretación clara de la zona de calor].
                    - **Veredicto de Coherencia:** [Alta/Baja] [Explicación médica].
                ...
                task_especialista = Task(
                    ...
                    1. **BÚSQUEDA AGRESIVA (RAG):**
                       ...
                       - Busca explícitamente términos como: "márgenes quirúrgicos melanoma", "tratamiento carcinoma basocelular", "seguimiento ganglio centinela".
                       - NO asumas nada. Busca la página exacta.
                    
                    2. **CRUCE DE INFORMACIÓN:**
                       - Toma el diagnóstico más probable (dado por la IA y el Auditor).
                       - Busca en los documentos recuperados qué se hace EXACTAMENTE con ese diagnóstico.
                    
                    3. **REDACCIÓN BIBLIOGRÁFICA:**
                       - Redacta el plan de acción de forma nutrida, profesional y profunda.
                       - **REGLA DE ORO 1:** Cada frase debe terminar con una cita tipo `[Fuente: Guía NCCN, Pág: X]`. SOLO usa documentos reales entregados por el buscador.
                       - **REGLA DE ORO 2:** ESTÁ ESTRICTAMENTE PROHIBIDO usar bloques de código (NO uses ```markdown ni ```). 
                       - **REGLA DE ORO 3:** ESTÁ ESTRICTAMENTE PROHIBIDO generar tablas, cuadros estadísticos, o texto en inglés. Tu respuesta debe ser ÚNICAMENTE texto continuo y viñetas (-).
                       - Si el buscador no devuelve info sobre un tema, di explícitamente: "No se encontró evidencia en los documentos disponibles sobre X".
                       
                    INSTRUCCIONES CRÍTICAS DE EJECUCIÓN (¡LEER ATENTAMENTE!):
                    1. ERES UN SISTEMA AUTOMATIZADO. NO me saludes, NO me hables, NO pidas más información ni instrucciones. GENERARÁS EL DICTAMEN INMEDIATAMENTE.
                    2. ESTÁS OBLIGADO a usar la herramienta `Buscador de Guías Clínicas` para extraer la información. Busca términos como: "márgenes quirúrgicos carcinoma", "manejo carcinoma basocelular".
                    3. Contrasta al menos 2 instituciones diferentes de los documentos (ej. NCCN, AAD, SEOM).
                    4. Redacta cada indicación con citas en formato APA: (Nombre Institución, Año, p. X).
                    
                    """,
                    agent=especialista_piel,
                    context=[task_atencion_primaria],
                    expected_output="""
                    Un documento Markdown puro con el Dictamen Oncológico. Sin texto conversacional antes ni después.
                    
                    Estructura obligatoria:

                    ### 1. Diagnóstico e Interpretación
                    - **Diagnóstico de Trabajo:** [Enfermedad validada por el auditor].
                    - **Justificación:** [Breve justificación clínica apoyada en la probabilidad IA].
                    
                    ### 2. Protocolo de Manejo (Evidencia Contrastada)
                    - **Manejo Recomendado:** [Instrucción médica] (Institución A, Año, p. X).
                    - **Márgenes Quirúrgicos:** [Medida] (Institución A, Año, p. X). En contraste, [Institución B] sugiere [Medida alterna] (Institución B, Año, p. Y).
                    - **Estudios Complementarios:** [Instrucción] (Institución, Año, p. X).

                    ### 3. Pronóstico y Seguimiento
                    - **Pauta de revisión:** [Tiempo]. [Fuente: Documento X, Pág Y]

                    ---
                    ### 📚 Referencias Bibliográficas
                    1. Documento X. (Año). *Título de la Guía*. Rango de páginas.
                    2. Documento X. (Año). *Título de la Guía*. Rango de páginas.
